...
async def chat_with_tools(
    ...
    # ── 从 SQLite 读取已上传文件（重启不丢失）──
    from agent.memory import _db_fetchone
    row = await _db_fetchone("SELECT file_text, file_name FROM sessions WHERE session_id=? AND active=1 AND file_text!=''", (session_id,))
    file_text = row[0] if row else ""
    file_name = row[1] if row else ""
    if file_text and session_id not in session_files:
        session_files[session_id] = file_text  # 回填内存缓存

    # 文件内容注入到用户消息
    if file_text and (force_tool == "doc_summary" or any(kw in message for kw in ["总结", "文档", "报告", "文件", "摘要", "这份", "这个", "版本", "指标", "提取"])):
        message = f"[已上传文件「{file_name}」内容如下]\n{file_text}\n[文件结束]\n\n用户问题：{message}"

    # 动态注入当前日期
    from datetime import datetime as dt
    today_str = dt.now().strftime("%Y年%m月%d日")
    dated_prompt = f"今天是{today_str}。\n\n{system_prompt}"
